...
class Dobot:
    ...
    # def move(self, x2, y2, z, r, radius=180, rX_pos=0, rY_pos=0, wait=False):
    #     response = self._get_pose()
    #     x1 = struct.unpack_from('f', response.params, 0)[0]
    #     y1 = struct.unpack_from('f', response.params, 4)[0]
    #     intersect = ((x2 - x1) * rX_pos + (y1 - y2) * rY_pos + (x1 - x2) * y1 + x1 * (y2 - y1))/math.sqrt(math.pow(x2-x1, 2) + math.pow(y1-y2, 2))
    #     new_radius = math.sqrt(math.pow(x2, 2) + math.pow(y2, 2))
    #     new_radius = -new_radius if x1 < 0 else new_radius
    #     current_radius = math.sqrt(math.pow(x1, 2) + math.pow(y1, 2))
    #     current_radius = -current_radius if x1 < 0 else current_radius
    ...
    #     # other = (abs(y2)-abs(y1) <= 1 and (x2 < radius and x1 < radius))
    #     # deltaY = abs(y2-y1)
    #     # deltaX = abs(x2-x1)
    #     # if deltaX > 10 or deltaY > 10:
    #     #     if (0 <= abs(intersect) < radius and not (-1 < intersect < 1)) or other:
    #     #         if y1 < -radius:
    #     #             self._set_ptp_cmd(radius+10, -radius-10, z, r, mode=PTPMode.MOVL_XYZ, wait=False)
    #     #             self._set_ptp_cmd(radius+10, radius+10, z, r, mode=PTPMode.MOVL_XYZ, wait=False)
    #     #         if y1 > radius:
    #     #             self._set_ptp_cmd(radius+10, radius+10, z, r, mode=PTPMode.MOVL_XYZ, wait=False)
    #     #         if y2 < -radius:
    #     #             self._set_ptp_cmd(radius+10, -radius-10, z, r, mode=PTPMode.MOVL_XYZ, wait=False)
    #         # elif -radius < intersect and not (-1 < intersect < 1) or other:
    #         #     # if y1 > radius:
    #         #     #     self._set_ptp_cmd(radius+10, radius+10, z, r, mode=PTPMode.MOVL_XYZ, wait=True)
    #             # self._set_ptp_cmd(radius+10, -radius-10, z, r, mode=PTPMode.MOVL_XYZ, wait=True)
    #             # if y2 > radius:
    #             #     self._set_ptp_cmd(radius+10, radius+10, z, r, mode=PTPMode.MOVL_XYZ, wait=True)
    #             # self._set_ptp_cmd(radius+10, -radius-10, z, r, mode=PTPMode.MOVL_XYZ, wait=True)
    #     self._set_ptp_cmd(x2, y2, z, r, mode=PTPMode.MOVL_XYZ, wait=wait)
